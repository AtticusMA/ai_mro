    ...
    public Result invoke(Invoker<?> invoker, Invocation invocation) throws RpcException {
        try {
            ...
            if ("true".equals(selfOnlyVal)) {
                ...
                if (deptIdsVal.startsWith("expand:")) {
                    // tooling-material 有 deptId 字段但无部门树查询能力，退化为本部门精确匹配
                    // manage-web 传来的是 deptId，直接解析根部门 id 做单值过滤
                    long rootDeptId = Long.parseLong(deptIdsVal.substring("expand:".length()));
                    DataScopeContext.setDeptIds(List.of(rootDeptId));
                } else {
